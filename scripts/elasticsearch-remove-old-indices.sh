...
INDICES_TEXT=`curl -u elastic:changeme -s "$ELASTICSEARCH/_cat/indices?pretty=true" | grep $GREP | sort -r | awk {'print $3'}`
...
if [ ${#INDEX[@]} -gt $KEEP ]; then
  for index in ${INDEX[@]:$KEEP};do
    ...
    if [ -n "$index" ]; then
      IDX=$(python3 -c "import urllib.parse; print(urllib.parse.quote_plus('${index}'))")
      if [ -z "$LOGFILE" ]; then
        curl -s -g -XDELETE -u elastic:changeme "$ELASTICSEARCH/${IDX}/" > /dev/null
      else
        echo `date "+[%Y-%m-%d %H:%M] "`" Deleting index: $index." >> $LOGFILE
        curl -s -g -XDELETE -u elastic:changeme "$ELASTICSEARCH/${IDX}/" >> $LOGFILE
      fi
    fi
  done
fi
